document.addEventListener("DOMContentLoaded", () => {
  ...
  // 8. OVERLAPPING TEAM SECTION SCROLL ANIMATION
  const teamSection = document.getElementById("equipe");
  const photoSara = document.getElementById("photo-sara");
  const photoValkiria = document.getElementById("photo-valkiria");
  const bioSara = document.getElementById("bio-sara");
  const bioValkiria = document.getElementById("bio-valkiria");

  if (teamSection && photoSara && photoValkiria && bioSara && bioValkiria) {
    window.addEventListener("scroll", () => {
      ...
      if (totalScroll > 0) {
        ...
        if (progress >= 0 && progress <= 1) {
          if (progress < 0.5) {
            // Sara Active
            if (!photoSara.classList.contains("active")) {
              photoSara.classList.add("active");
              bioSara.classList.add("active");
              
              photoValkiria.classList.remove("active");
              bioValkiria.classList.remove("active");
            }
          } else {
            // Valkiria Active
            if (!photoValkiria.classList.contains("active")) {
              photoValkiria.classList.add("active");
              bioValkiria.classList.add("active");
              
              photoSara.classList.remove("active");
              bioSara.classList.remove("active");
            }
          }
        } else if (progress < 0) {
          // Reset to Sara active if above section
          photoSara.classList.add("active");
          bioSara.classList.add("active");
          photoValkiria.classList.remove("active");
          bioValkiria.classList.remove("active");
        } else if (progress > 1) {
          // Keep Valkiria active if below section
          photoValkiria.classList.add("active");
          bioValkiria.classList.add("active");
          photoSara.classList.remove("active");
          bioSara.classList.remove("active");
